...
def plot_second_graph(k, colormap1, colormap2,n,k1,k2):
    """
    ...
    :param n: how many functions to plot
    :param k1: color of area
    :param k2: another color of area
    """
    fig = plt.figure()
    ax = fig.add_subplot(111)
    xax = ax.xaxis
    plt.ylim(0, 25) # limit the y-scale
...
def plot_a_function(ax, i, k_array, v, color,n):
...
def fill_areas(ax,k,k_array, v, colormap1,colormap2,n,k1,k2):
    ...
    for i in range(-2, n-2, 1):
        ax.fill_between(x, ((1 - k_array[2 + i] / v) - x) / (x * (k_array[2 + i] - 1)), 0,
                        facecolor=colormap1(k1)) #40 + i * 30

    for i in range(k-2, -1, -1):
        ax.fill_between(x, ((1 - k_array[i] / v) - x) / (x * (k_array[i] - 1)), 25,
                        facecolors=colormap2(k2)) #120 + i*30
